...
* each colum represents a single individual
...
represent an individual and each column a SNP) and transposed using a
...
class KinshipWriter(object):
    ...
    def __init__(self, mset, path, base_filename = 'bl-vl-kinship',
        ...
        self.mset = mset
        self.out_k_file = open(os.path.join(path, '%s.kinship' % base_filename),
                               'w')
        self.out_k_csvw = csv.writer(self.out_k_file, delimiter='\t')
        self.out_i_file = open(os.path.join(path, '%s.inds' % base_filename),
                               'w')
        self.out_i_csvw = csv.writer(self.out_i_file, delimiter='\t')
    ...
    def write_record(self, individual, data_collection_samples = None):
        ...
        if len(dsamples) > 0:
            ds = dsamples[0] # Right now, just ignore all the other data samples
            self.out_i_csvw.writerow([individual.id])
...
def main(argv):
    ...
    for ind in inds:
        logger.debug('Writing record for individual %s' % ind.id)
