...
class Proxy(ProxyCore):
  ...
  def create_snp_marker_definitions_table(self):
    self.create_table(self.SNP_MARKER_DEFINITIONS_TABLE, self.SNP_MARKER_DEFINITIONS_COLS)
  ...
  def add_snp_marker_definitions(self, stream, op_vid, batch_size=50000):
    ...
    self.add_table_rows_from_stream(self.SNP_MARKER_DEFINITIONS_TABLE, i_s, batch_size)
    return vids
  ...
  def get_snp_marker_definitions(self, selector=None, batch_size=50000):
    ...
    """
    return self.get_table_rows(self.SNP_MARKER_DEFINITIONS_TABLE, selector, batch_size)

  #-- marker sets
  def create_snp_markers_set_table(self):
    self.create_table(self.SNP_SET_DEF_TABLE, self.SNP_SET_DEF_COLS)
  ...
  def get_snp_markers_sets(self, selector=None, batch_size=50000):
    return self.get_table_rows(self.SNP_SET_DEF_TABLE, selector, batch_size)
  ...
  def fill_snp_markers_set(self, set_vid, stream, op_vid, batch_size=50000):
    ...
    self.add_table_rows_from_stream(self.SNP_SET_TABLE, i_s, batch_size)
    return N[0]

  def get_snp_markers_set(self, selector=None, batch_size=50000):
    return self.get_table_rows(self.SNP_SET_TABLE, selector, batch_size)

  #-- alignment
  def create_snp_alignment_table(self):
    self.create_table(self.SNP_ALIGNMENT_TABLE, self.SNP_ALIGNMENT_COLS)
  ...
  def add_snp_alignments(self, stream, op_vid, batch_size=50000):
    ...
    return self.add_table_rows_from_stream(self.SNP_ALIGNMENT_TABLE, i_s, batch_size)

  def get_snp_alignments(self, selector=None, batch_size=50000):
    return self.get_table_rows(self.SNP_ALIGNMENT_TABLE, selector, batch_size)
  ...
  def create_gdo_repository(self, set_vid, N):
    ...
    self.create_table(table_name, self.SNP_GDO_REPO_COLS(N))
    return set_vid
  ...
  def __unwrap_gdo(self, set_id, row):
    ...
    #--
    p = np.fromstring(self.__normalize_size(row['probs'], row.dtype['probs'].itemsize),
                      dtype=np.float32)
  ...
  def get_gdo(self, set_vid, vid):
    table_name = self._gdo_table_name(set_vid)
    rows = self.get_table_rows(table_name, selector='(vid == "%s")' % vid)
    assert len(rows) == 1
  ...
  def get_gdo_iterator(self, set_vid, batch_size=100):
    ...
    table_name = self._gdo_table_name(set_vid)
    return iterator(self.get_table_rows_iterator(table_name, batch_size))
