...
class Recorder(core.Core):
    ...
    def record(self, records, otsv, rtsv, blocking_validation):
        ...
        if blocking_validation and len(bad_records) >= 1:
            raise core.ImporterValidationError(
                '%d invalid records' % len(bad_records))
        act_setups = set((r['source'], r.get('device', None),
                          Recorder.get_action_setup_options(r,
    ...
    def do_consistency_checks(self, records):
        ...
        if self.seq_sample_klass == self.kb.RawSeqDataSample:
            good_recs, brecs = self.do_consistency_checks_raw_seq_data_sample(
                good_recs)
            bad_recs.extend(brecs)
        if self.seq_sample_klass == self.kb.SeqDataSample:
            good_recs, brecs = self.do_consistency_checks_seq_data_sample(
                good_recs)
            bad_recs.extend(brecs)
        if self.seq_sample_klass == self.kb.AlignedSeqDataSample:
            good_recs, brecs = self.do_consistency_checks_aligned_seq_data_sample(
                good_recs)
            bad_recs.extend(brecs)
    ...
    def do_consistency_checks_common_fields(self, records):
        ...
        for i, r in enumerate(records):
            ...
            if self.missing_fields(mandatory_fields, r):
                ...
                continue
            if r['label'] and self.is_known_object_label(r['label'],
                                                         self.kb.DataSample):
        ...
        self.logger.info('done common fields consistency checks')
    ...
    def do_consistency_checks_seq_data_sample(self, records):
        ...
        for i, r in enumerate(records):
            reject = 'Rejecting import of line %d.' % i
            if r['sample'] and not self.is_known_object_id(r['sample'],
                                                           self.kb.Tube):
    ...
    def do_consistency_checks_aligned_seq_data_sample(self, records):
        ...
        for i, r in enumerate(records):
            ...
            if r['sample'] and not self.is_known_object_id(r['sample'],
                ...
                bad_records.append(bad_rec)
                continue
            if r['genome_reference'] and not self.is_known_object_id(
                r['genome_reference'], self.kb.ReferenceGenome):
                m = 'unknown reference genome with ID %s. ' % r[
                    'genome_reference']
                self.logger.warning(m + reject)
                bad_rec = copy.deepcopy(r)
                bad_rec['error'] = m
            ...
            good_records.append(r)
        self.logger.info(
            'done AlignedSeqDataSample specific consistency checks')
        return good_records, bad_records
    ...
    def process_chunk(self, otsv, chunk, actions, study):
        ...
        for r in chunk:
            ...
            if self.seq_sample_klass == self.kb.SequencerOutput:
                seq_data_samples.append(
                    self.conf_sequencer_output_data_sample(r, a))
                ...
                self.logger.error(
                    'Unmanaged data sample type %r' % self.seq_sample_klass)
                sys.exit(
                    'Unmanaged data sample type %r' % self.seq_sample_klass)
    ...
    def conf_raw_seq_data_sample(self, r, a):
        conf = {'label': r['label'],
                ...
                'action': a}
        if r['lane']:
            conf['lane'] = self.kb.get_by_vid(self.kb.Lane, r['lane'])
    ...
    def conf_seq_data_sample(self, r, a):
        conf = {'label': r['label'],
                ...
                'action': a}
        if r['sample']:
            conf['sample'] = self.kb.get_by_vid(self.kb.Tube, r['sample'])
    ...
    def conf_aligned_seq_data_sample(self, r, a):
        ...
        if r['genome_reference']:
            conf['referenceGenome'] = self.kb.get_by_vid(self.kb.ReferenceGenome,
                                                         r['genome_reference'])
