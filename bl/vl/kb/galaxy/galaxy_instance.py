...
class GalaxyInstance(object):
    ...
    def get_workflow(self, workflow_id, id_by_name=None):
        ...
        if id_by_name is None:
            wf_list = self.blend.gi.workflows.get_workflows()
            id_by_name = map_name_to_id(wf_list)
        root, instance = parse_workflow_name(wf['name'])
        self.logger.debug('%s %s' % (root, instance))
        if root not in id_by_name:
            self._raise_exception(RuntimeError,
                                  'Missing root workflow %s' % root)
        if len(id_by_name[root]) != 1:
            self._raise_exception(RuntimeError,
                                  'Multiple root workflows for %s' % root)
        root_id = id_by_name[root][0]
    ...
    def save(self, history, metastore=None):
        pass
    ...
    def _check_input_compatibility(self, input_object, in_port):
    ...
    def run(self, workflow, input_object, wait=True):
        """
        Run 'workflow' with 'input_object', if 'wait' (default behavior)
        then wait for the results, else put request in queue and exit.
        It will return the related History object.
        """
        in_port = workflow.ports['inputs'].values()[0]
        input_paths = self._get_input_paths(input_object, in_port)
        return self.run_workflow(workflow, input_paths, wait)
